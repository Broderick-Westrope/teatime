...
func TestDeriveKey(t *testing.T) {
	tt := map[string]struct {
		...
		"key length 24": {
			...
			encodedParams: "$argon2id$v=19$m=65536,t=1,p=10$c29tZV90ZXN0X3NhbHQ",
			keyLenght:     24,
			wantKey: []byte{250, 127, 55, 191, 157, 157, 158, 167, 112, 237, 192, 78, 113, 49,
				170, 62, 182, 54, 206, 187, 169, 103, 207, 232},
		},
		"key length 32": {
			password:      "pa$$word",
			encodedParams: "$argon2id$v=19$m=65536,t=1,p=10$c29tZV90ZXN0X3NhbHQ",
			keyLenght:     32,
			wantKey: []byte{224, 155, 147, 58, 185, 138, 18, 132, 178, 70, 157, 131, 177, 128,
				182, 123, 43, 195, 131, 39, 29, 214, 67, 196, 29, 234, 255, 166, 211, 9, 95, 191},
		},
	}

	for name, tc := range tt {
		t.Run(name, func(t *testing.T) {
			key, err := secure.DeriveKey(tc.password, tc.encodedParams, tc.keyLenght)
...
func TestCreateKey(t *testing.T) {
	...
	key, hash, err := secure.CreateKey("pa$$word", inParams, 16)
	require.NoError(t, err)

	params, salt, err := secure.DecodeArgonParams(hash)
	require.NoError(t, err)
...
func TestDecryptionFlow(t *testing.T) {
	...
	key, err := secure.DeriveKey(password, encodedParams, 24)
	require.NoError(t, err)
